...
export const afterAuthRedirect = (encryptedToken: string) => {
  const source = window.sessionStorage.getItem(SESSION_STORAGE_KEYS.SOURCE);

  if (source === 'myaccount') {
    window.location.href = `https://staging-youraccount.zestmoney.in/authentication?token=${encryptedToken}`;
  } else if (source === 'cmj') {
    window.location.href = `https://staging-app.zestmoney.in/?token=${encryptedToken}`;
  } else if(source === 'payment') {
    window.location.href = `https://staging-pay.zestmoney.in/loggedinredirect?token=${encryptedToken}`;
  }
}